...
export default function CancelModal({ onClose }: CancelModalProps) {
  ...
  const { data: reasons } = GETRequest<Reasons[]>(`/reasons`, 'reasons', [lang]);
          ...
          onSubmit={async values => {
            ...
            if (userStr) {
              ...
              axiosInstance
                ...
                .then(() => {
                  toast.success(tarnslation?.success_cancel_toast ?? '');
                  queryClient.invalidateQueries({
                    queryKey: ['getOrders'],
                  });
                  navigate(
                    `/${lang}/${ROUTES.orders[lang as keyof typeof ROUTES.orders]}`,
                  );
                })
                .catch(error => {
            ...
            <Form className="space-y-4">
              <div className="relative">
                ...
                >
                  ...
                  <option value={'other'}>{tarnslation?.Other}</option>
                </Field>
              ...
              {values.reason === 'other' && (
                <div className="relative">
                  <Field
                    as="textarea"
                    name="comment"
                    placeholder={tarnslation?.reyiniz_key ?? ''}
