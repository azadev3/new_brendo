...
const SearchableSelect = ({ regionData, onChange, value }: { regionData: any, onChange: any, value?: any }) => {
    ...
    React.useEffect(() => {
        if (value && regionData.length > 0) {
            const selected = regionData.find((r:any) => r.regionId === value || r.id === value);
            if (selected) {
            ...
            {showDropdown && (
                <ul className="absolute z-10 w-full bg-white border border-gray-200 max-h-60 overflow-auto rounded-lg mt-1 shadow">
                    {filteredRegions.length > 0 ? (
                        ...
                        <li className="px-4 py-2 text-gray-500">{translation?.Ничего_не_найдено}</li>
                    )}
                </ul>
            )}
        </div>
