  ...
  const handleSaveAddress = async () => {
    ...
    try {
      ...
      const res = await fetch(
        "https://admin.brendoo.com/api/changeOrderAddress",
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          text || `Не удалось обновить адрес. HTTP ${res.status}`
        );
      }
          ...
          {changeAddress && (
            ...
            >
              <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
                ...
                {addrError && (
                  <p className="text-sm text-red-600 mb-2">{addrError}</p>
                )}
